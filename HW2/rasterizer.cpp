...
static bool insideTriangle(int x, int y, const Vector3f* _v)
{
    ...
    Eigen::Vector3f AB=B-A; Eigen::Vector3f AP=P-A;
    Eigen::Vector3f BC=C-B; Eigen::Vector3f BP=P-B;
    Eigen::Vector3f CA=A-C; Eigen::Vector3f CP=P-C;

    float z1 = AB.cross(AP).z();
    float z2 = BC.cross(BP).z();
    float z3 = CA.cross(CP).z();

    bool isInside = (z1>0 && z2>0 && z3>0) || (z1<0 && z2<0 && z3<0);
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    // TODO : Find out the bounding box of current triangle. Axis-aligned bounding box
            ...
            for(int y=aabb_miny; y<=aabb_maxy; y++)
            {
                ...
                for (int i=0; i<4; i++)
                {
                    if(insideTriangle(x+pos[i].x(), y+pos[i].y(), t.v))
                    {
                        ...
                        minDepth = std::min(minDepth, abs(z_interpolated));
                        count++;
                    }
                }
                if (count!=0)
        ...
        for (int x=(int)aabb_minx; x<=aabb_maxx; x++)
        {
            for (int y=(int)aabb_miny; y<=aabb_maxy; y++)
            {
                ...
                if(isInside){
                    ...
                    if (depth_buf[id] > abs(z_interpolated)){
                        Eigen::Vector3f color = t.getColor();
                        set_pixel(Vector3f(x,y,1.0f), color);
                        //depth
                        depth_buf[id] = abs(z_interpolated);
                    }
